<script lang="ts">
	...
	import ElementSelector, { type SelectedElement } from './overlays/ElementSelector.svelte'
	...
	let { html, studioState, currentStage = null, generatedCode = '', onReady, onMetrics, onElementSelect }: Props = $props()
	...
	<div class="flex-1 relative overflow-hidden">
		...
		{#if !html && studioState === 'idle'}
			<div class="absolute inset-0 flex flex-col items-center justify-center text-center p-8 animate-fade-in">
				<svg class="w-20 h-20 text-[var(--color-text-muted)]/30 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
				...
				<p class="text-sm text-[var(--color-text-muted)]">
					Type a prompt in the chat panel to generate your first component
				</p>
			</div>
		{/if}
